...
def add_sessionname(csv_path):
    ...
    df["session_letter"] = df["linguaskill_type"].str.split().str[1].str[0]
    pattern = df["exam_type"].map(name_dic)  # ATTENTION: ton dict correspond à exam_type, pas linguaskill_type

    df["skill_code"] = np.where(
        pattern.notna() & df["session_letter"].notna(),
        pattern.str.split("S", n=1).str[0] + "S" + df["session_letter"] + pattern.str.split("S", n=1).str[1],
        np.nan
    )
    df["session_name"] = df["exam_date"] + ' ' + df["skill_code"] + " " + df["email"]
    df.to_csv(csv_path)